...
`watchlist.json` に登録した日本株の15分確定足で、パラボリックSARの
上転換を検知してDiscordに通知するシステムです。
...
↓
GitHub Actionsが平日の取引時間帯に5分ごとに監視
↓
パラボリック上転換を検知
↓
Discordに通知
...
- 未確定の15分足は判定しません。
- Yahoo Financeの反映が1回遅れる場合を考慮しつつ、確定から20分を超えた
  古い足は新規通知しません。
- 通知済みの転換足は `alert_state.json` に記録し、重複通知を防ぎます。
