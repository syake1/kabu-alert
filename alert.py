...
def prepare_data(code):
    ...
    # 現在進行中の15分足を除外する。
    now = now_jst()
    closed_until = now - timedelta(minutes=15)
    hist = hist[hist.index + timedelta(minutes=15) <= now]
    if hist.empty:
        return None

    # 取引時間外に混ざったデータを極力除外。
    hist = hist[(hist.index.time >= datetime.strptime("09:00", "%H:%M").time()) &
                (hist.index.time <= datetime.strptime("15:15", "%H:%M").time())]
    if len(hist) < 30:
        return None
...
def find_recent_buy_transition(hist):
    """当日の確定15分足だけを対象に、直近のSAR下→上転換を探す。"""
    ...
    day = day.tail(LOOKBACK_BARS + 1)
    trend = day["SAR_trend"].astype(int).tolist()
...
def send_discord(code, name, bar, state):
    if not DISCORD_WEBHOOK:
        print("❌ DISCORD_WEBHOOK が設定されていません")
        return False

    bar_time = bar.name
    state_key = f"{code}_buy_sar"
    bar_key = bar_time.isoformat()

    if state.get(state_key) == bar_key:
        print(f"  → {code} は通知済み: {bar_time.strftime('%m/%d %H:%M')}")
        return False
    ...
    try:
        ...
        print(f"✅ Discord通知: {code} {name} / {bar_time.strftime('%m/%d %H:%M')}")
        return True
    except Exception as e:
...
def main():
    ...
    print("対象: watchlist.jsonに登録された銘柄のみ / 15分足確定足のみ")
    ...
    for stock in watchlist:
        ...
        bar = find_recent_buy_transition(hist)
        if bar is None:
            latest = hist.iloc[-1]
            trend_text = "上向き" if int(latest["SAR_trend"]) == 1 else "下向き"
            print(f"  → 買い転換なし（最新確定足 {latest.name.strftime('%H:%M')} / SAR {trend_text}）")
            continue

        print(f"  → 🎯 買い転換検出: {bar.name.strftime('%Y/%m/%d %H:%M')} JST")
        if send_discord(code, name, bar, state):
            alert_count += 1
